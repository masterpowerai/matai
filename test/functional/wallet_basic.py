...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 210 MTAI from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 1000 MTAI in block rewards plus fees, but
        ...
        # Send 100 MTAI with subtract fee from amount
        ...
        # Sendmany 100 MTAI with subtract fee from amount
        ...
        # Sendmany with explicit fee (MTAI/kB)
        ...
        assert_raises_rpc_error(-8, "Selected estimate_mode requires a fee rate",
            ...
            amounts={ address: 10 },
            estimate_mode='masterpowerai/kB')
        ...
        # This will raise an exception for attempting to get the private key of an invalid masterpowerai address
        assert_raises_rpc_error(-5, "Invalid masterpowerai address", self.nodes[0].dumpprivkey, "invalid")

        # This will raise an exception for attempting to set a label for an invalid masterpowerai address
        assert_raises_rpc_error(-5, "Invalid masterpowerai address", self.nodes[0].setlabel, "invalid address", "label")

        # This will raise an exception for importing an invalid address
        assert_raises_rpc_error(-5, "Invalid masterpowerai address or script", self.nodes[0].importaddress, "invalid")
        ...
        # send with explicit masterpowerai/kb fee
        self.log.info("test explicit fee (sendtoaddress as masterpowerai/kb)")
        self.nodes[0].generate(1)
        self.sync_all(self.nodes[0:3])
