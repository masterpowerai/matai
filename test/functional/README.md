...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x500=12500 MTAI) in its wallet. Using them is much more
...
with the masterpoweraid(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a masterpoweraid.  `P2PInterface`
...
Basic code to support P2P connectivity to a masterpoweraid.
