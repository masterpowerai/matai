...
The masterpowerai Core daemon can be configured to act as a trusted "border
router", implementing the masterpowerai wire protocol and relay, making
...
The ZeroMQ feature in masterpowerai Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building masterpoweraid:
...
For instance:
    ...
    $ masterpoweraid -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/masterpoweraid.tx.raw \
...
These options can also be provided in masterpowerai.conf.
...
From the perspective of masterpoweraid, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into masterpoweraid directly. Furthermore, no information is
...
using. masterpoweraid appends an up-counting sequence number to each
