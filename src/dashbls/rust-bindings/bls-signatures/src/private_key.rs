...
impl PrivateKey {
    ...
    #[cfg(feature = "masterpowerai_helpers")]
    pub fn generate_masterpowerai(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "masterpowerai_helpers")]
    pub fn generate_masterpowerai_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_masterpowerai(rng))
            .collect()
    }
