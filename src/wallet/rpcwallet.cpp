...
// Copyright (c) 2024 The masterpowerai Core developers
...
static const uint32_t WALLET_MTAI_KB_TO_DUFF_B = COIN / 1000; // 1 duff / B = 0.00001 MTAI / kB
...
static void SetFeeEstimateMode(const CWallet* pwallet, CCoinControl& cc, const UniValue& estimate_mode, const UniValue& estimate_param)
{
    ...
    if (cc.m_fee_mode == FeeEstimateMode::MTAI_KB || cc.m_fee_mode == FeeEstimateMode::DUFF_B) {
        ...
        if (cc.m_fee_mode == FeeEstimateMode::DUFF_B) {
            fee_rate /= WALLET_MTAI_KB_TO_DUFF_B;
        }
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    RPCHelpMan{"getnewaddress",
        "\nReturns a new masterpowerai address for receiving payments.\n"
        ...
        RPCResult{
    RPCResult::Type::STR, "address", "The new masterpowerai address"
        },
        RPCExamples{
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    RPCHelpMan{"getrawchangeaddress",
        "\nReturns a new masterpowerai address, for receiving change.\n"
...
static UniValue setlabel(const JSONRPCRequest& request)
{
    RPCHelpMan{"setlabel",
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The masterpowerai address to be associated with a label."},
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid masterpowerai address");
    }
...
static UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{"sendtoaddress",
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The masterpowerai address to send to."},
    ...
    "                             The recipient will receive less amount of masterpowerai than you enter in the amount field."},
        ...
        RPCResult{
            RPCResult::Type::ARR, "", "",
            {
                {RPCResult::Type::ARR, "", "",
                {
                    {RPCResult::Type::ARR, "", "",
                    {
                        {RPCResult::Type::STR, "address", "The masterpowerai address"},
...
static UniValue listaddressbalances(const JSONRPCRequest& request)
{
    RPCHelpMan{"listaddressbalances",
        ...
        RPCResult{
            RPCResult::Type::ARR, "", "",
            {
                {RPCResult::Type::STR_AMOUNT, "amount", "The masterpowerai address and the amount in " + CURRENCY_UNIT},
            }
        },
        RPCExamples{
...
static UniValue signmessage(const JSONRPCRequest& request)
{
    RPCHelpMan{"signmessage",
        "\nSign a message with the private key of an address" +
                HELP_REQUIRING_PASSPHRASE,
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The masterpowerai address to use for the private key."},
...
static UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    RPCHelpMan{"getreceivedbyaddress",
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The masterpowerai address for transactions."},
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    RPCHelpMan{"sendmany",
                        ...
                        HELP_REQUIRING_PASSPHRASE,
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, RPCArg::Optional::NO, "The addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The masterpowerai address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
            ...
            "                           Those recipients will receive less masterpowerai than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid masterpowerai address: ") + name_);
        }
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    RPCHelpMan{"addmultisigaddress",
        ...
        "Each key is a masterpowerai address or hex-encoded public key.\n"
        "This functionality is only intended for use with non-watchonly addresses.\n"
        "See `importaddress` for watchonly p2sh address support.\n"
        "If 'label' is specified, assign address to that label.\n",
        {
            {"nrequired", RPCArg::Type::NUM, RPCArg::Optional::NO, "The number of required signatures out of the n keys or addresses."},
            {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "The masterpowerai addresses or hex-encoded public keys",
                {
                    {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "masterpowerai address or hex-encoded public key"},
                },
                },
        ...
        RPCResult{
            RPCResult::Type::ARR, "", "",
            {
                {RPCResult::Type::OBJ, "", "", Cat(Cat<std::vector<RPCResult>>(
                    {
                        {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction"},
                        {RPCResult::Type::STR, "address", "The masterpowerai address of the transaction. Not present for\n"
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::ARR, "transactions", "",
                {
                    ...
                    {
                        {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction"},
                        {RPCResult::Type::STR, "address", "The masterpowerai address of the transaction."},
                        {RPCResult::Type::STR, "category", "The transaction category.\n"
                            "\"send\"                  Transactions sent.\n"
                            "\"coinjoin\"              Transactions sent using CoinJoin funds.\n"
                            "\"receive\"               Non-coinbase transactions received.\n"
                    ...
                    {RPCResult::Type::ARR, "details", "",
                        {
                            {RPCResult::Type::OBJ, "", "",
                                {
                                    {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction"},
                                    {RPCResult::Type::STR, "address", "The masterpowerai address involved in the transaction."},
                                    {RPCResult::Type::STR, "category", "The transaction category.\n"
                                        "\"send\"                  Transactions sent.\n"
                                        "\"coinjoin\"              Transactions sent using CoinJoin funds.\n"
                                        "\"receive\"               Non-coinbase transactions received.\n"
...
static UniValue walletpassphrase(const JSONRPCRequest& request)
{
    RPCHelpMan{"walletpassphrase",
        ...
        "This is needed prior to performing transactions related to private keys such as sending masterpowerai\n"
        "\nNote:\n"
...
static UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    "\nNow set the passphrase to use the wallet, such as for signing or sending masterpowerai\n"
...
static UniValue lockunspent(const JSONRPCRequest& request)
{
    RPCHelpMan{"lockunspent",
        ...
        "A locked transaction output will not be chosen by automatic coin selection, when spending masterpowerai.\n"
...
static UniValue loadwallet(const JSONRPCRequest& request)
{
    RPCHelpMan{"loadwallet",
        ...
        "\nNote that all wallet command-line options used when starting masterpoweraid will be"
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{"listunspent",
        ...
        "Optionally filter to only include txouts paid to specified addresses.\n",
        {
            ...
            {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "The masterpowerai addresses to filter",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "masterpowerai address"},
                },
            },
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid masterpowerai address: ") + input.get_str());
            }
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid masterpowerai address");
            }
    ...
    RPCHelpMan{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* default */ "pool address", "The masterpowerai address to receive the change"},
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    RPCHelpMan{"getaddressinfo",
        "\nReturn information about the given masterpowerai address.\n"
        "Some of the information will only be present if the address is in the active wallet.\n",
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The masterpowerai address for which to get information."},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::STR, "address", "The masterpowerai address validated."},
        ...
        "Implements the Creator and Updater roles.\n",
        {
            ...
            {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "The outputs (key-value pairs), where none of the keys are duplicated.\n"
                    ...
                    "                             accepted as second parameter.",
                {
                    {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the masterpowerai address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                        },
                        },
                    {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                        {
            ...
            {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                {
                    {"changeAddress", RPCArg::Type::STR_HEX, /* default */ "pool address", "The masterpowerai address to receive the change"},
                    {"changePosition", RPCArg::Type::NUM, /* default */ "random", "The index of the change output"},
                    {"includeWatching", RPCArg::Type::BOOL, /* default */ "true for watch-only wallets, otherwise false", "Also select inputs which are watch only"},
                    {"lockUnspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
                    {"feeRate", RPCArg::Type::AMOUNT, /* default */ "not set: makes wallet determine the fee", "Set a specific fee rate in " + CURRENCY_UNIT + "/kB"},
