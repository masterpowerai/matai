 ...
 * database.
 *
 * CBlock(hash=00000ffd590b14, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=e0028e, nTime=1707921145, nBits=1e0ffff0, nNonce=26713, vtx=1)
...
{
    const char* pszTimestamp = "MASTERPOWERAI 14022024";
    const CScript genesisOutputScript = CScript() << ParseHex("04995d49982f7b435b69788d023af73c0576182a57a5a82fe6c587578d8921d1f206c09d2e0495c1aa0a479cfff4e36e734ce0fe788c01bebe412a04b018f66602") << OP_CHECKSIG;
...
public:
    CMainParams() {
        ...
        consensus.nMasternodeMinimumConfirmations = 52;
        ...
        consensus.defaultAssumeValid = uint256S("0x00000200cf159f0d116c0a4e97153954f8db249504e57dbe5c8590f0213a6a28"); 
        ...
        genesis = CreateGenesisBlock(1707921145, 26713, 0x1e0ffff0, 1, 200 * COIN);
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x00000200cf159f0d116c0a4e97153954f8db249504e57dbe5c8590f0213a6a28"));
        assert(genesis.hashMerkleRoot == uint256S("0x1ae02c21e12b297187bf673f1d9848c8fc8b6209c7d633a4bed91eed6d8ccb7a"));
        ...
        // masterpowerai BIP32 pubkeys start with 'xpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x88, 0xB2, 0x1E};
        // masterpowerai BIP32 prvkeys start with 'xprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x88, 0xAD, 0xE4};

        // masterpowerai BIP44 coin type is '5'
        nExtCoinType = 5;
        ...
        vSporkAddresses = {"MMUYoFEP5FV8yqN2eLxF3DwRFuw8PwrBdh"};
        nMinSporkKeys = 1;

        checkpointData = {
            {
                {0, uint256S("0x00000200cf159f0d116c0a4e97153954f8db249504e57dbe5c8590f0213a6a28")},
            }
        };
        ...
        // getchaintxstats 17280 00000200cf159f0d116c0a4e97153954f8db249504e57dbe5c8590f0213a6a28
        chainTxData = ChainTxData{
                1707921145, // * UNIX timestamp of last known number of transactions
...
public:
    CTestNetParams() {
        ...
        // By default assume that the signatures in ancestors of this block are valid.
        consensus.defaultAssumeValid = uint256S("0x000006e9f43ba76442d7505769ea3f248c37e58792c5600b6cc47740f466746c"); 
        ...
        genesis = CreateGenesisBlock(1707921150UL, 221011UL, 0x1e0ffff0, 1, 200 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x000006e9f43ba76442d7505769ea3f248c37e58792c5600b6cc47740f466746c"));
        assert(genesis.hashMerkleRoot == uint256S("0x1ae02c21e12b297187bf673f1d9848c8fc8b6209c7d633a4bed91eed6d8ccb7a"));
        ...
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,178);
        // Testnet masterpowerai BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Testnet masterpowerai BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Testnet masterpowerai BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
        ...
        vSporkAddresses = {"M9z6WPgPPa2LDjNwN4nsRauHb3CwxYjTS4"};
        nMinSporkKeys = 1;

        checkpointData = {
            {
                {0, uint256S("0x000006e9f43ba76442d7505769ea3f248c37e58792c5600b6cc47740f466746c")},
            }
        };
        ...
        // getchaintxstats 17280 000006e9f43ba76442d7505769ea3f248c37e58792c5600b6cc47740f466746c
        chainTxData = ChainTxData{
                1707921150, // * UNIX timestamp of last known number of transactions
                0,    // * total number of transactions between genesis and that timestamp
                            //   (the tx=... number in the ChainStateFlushed debug.log lines)
                0,       // * estimated number of transactions per second after that timestamp
        };
...
public:
    explicit CDevNetParams(const ArgsManager& args) {
        ...
        genesis = CreateGenesisBlock(1707921155, 0, 0x207fffff, 1, 200 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x0f33812953c5266fd0ddc4d530d199e15ac9e0d972c840b399be61596060b6cf"));
        assert(genesis.hashMerkleRoot == uint256S("0x1ae02c21e12b297187bf673f1d9848c8fc8b6209c7d633a4bed91eed6d8ccb7a"));
        ...
        checkpointData = (CCheckpointData) {
            {
                { 0, uint256S("0x0f33812953c5266fd0ddc4d530d199e15ac9e0d972c840b399be61596060b6cf")},
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        // privKey: 6yFHBdSaLLEH2u944bsrfUa9iGvqcTt2RPyB8AGwr9Qh8ZHtiP8
        vSporkAddresses = {"MRPToxGWRLm59f8VaAuQSCWWaRPRr1RHG6"};
        nMinSporkKeys = 1;
        ...
        // Regtest masterpowerai addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,50);
        // Regtest masterpowerai script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,51);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,178);
        // Regtest masterpowerai BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Regtest masterpowerai BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Regtest masterpowerai BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
