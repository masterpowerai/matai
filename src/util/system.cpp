...
const char * const BITCOIN_CONF_FILENAME = "masterpowerai.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //masterpowerai-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\masterpowerai
    // macOS: ~/Library/Application Support/masterpowerai
    // Unix-like: ~/.masterpowerai
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "masterpowerai";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/masterpowerai";
#else
    // Unix-like
    return pathRet / ".masterpowerai";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty masterpowerai.conf if it does not exist
